...
const CurrencyCard: React.FC<CurrencyCardProps> = ({ type, amount, flag, currency, tailPosition = 'top' }) => {
  ...
  return (
    <div className="relative group">
      <div className="flex items-center gap-3 bg-white p-3 rounded-2xl shadow-lg min-w-[180px] hover:scale-105 transition-transform duration-300 cursor-default relative z-10">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isSent ? 'bg-red-100' : 'bg-purple-100'}`}>
          <Gift className={`w-5 h-5 ${isSent ? 'text-red-500' : 'text-[#6c5ce7]'}`} />
        </div>
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-base text-[#18181B] font-bold">{currency}</span>
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="relative w-full aspect-[1.8/1] mx-auto">
        <Image src={MapImage} alt='Global map visualization' className="opacity-80" />
